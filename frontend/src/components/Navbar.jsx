    ...
    <>
      <header className="flex items-center justify-between bg-slate-900 text-cyan-600 px-3 sm:px-6 py-4 font-serif ">
        ...
        <div className="flex items-center justify-between gap-2 mr-4">
          ...
          <div className="border border-cyan-500 rounded-lg p-2 shadow-sm shadow-cyan-600 hover:shadow-md cursor-pointer relative">
            <MenuIcon
              onClick={() => {
                open ? setOpen(false) : setOpen(true);
              }}
              className="size-4 sm:size-7 text-cyan-400 font-bold"
            ></MenuIcon>
            {open && (
              <div className="flex flex-col gap-1 text-sm text-cyan-400 bg-slate-800 p-1 rounded-lg absolute left-6 top-8 sm:top-11">
                <button
                  onClick={() => {
                    setState("all");
                  }}
          ...
          <div
            ...
            className="flex items-center justify-center bg-white rounded-full h-9 w-9 sm:w-12 sm:h-12 text-xl relative cursor-pointer"
          >
            {userData && userData.name ? userData.name[0].toUpperCase() : "?"}
            {logoutOpen && (
              <div className="absolute text-sm top-10 right-2 text-black bg-white p-2">
                <ul className="w-24">
                  <li
                    onClick={logout}
